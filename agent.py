...
8. **重要：在 thought 中记录所有已获取的关键信息**（如餐厅名、地址、路线起终点等）。因为截图内容不会保留到后续步骤，只有 thought 中的文字会被记住
9. 在高德地图做路线规划时：点击"路线"按钮后，修改起点和终点输入框，然后切换到对应的出行方式标签（骑行/驾车/公交）。必须在高德地图上实际看到距离和时间数据后才能返回结果，不要自己编造数据
...
class PhoneAgent:
    ...
    def choose_model(self, step: int) -> str:
        return MODEL_SMART
    ...
    def compress_messages(self, messages: list) -> list:
        ...
        # 找到安全的裁切点：确保保留的消息中 tool response 都有对应的 tool_call
        tail = messages[-8:]
        # 如果 tail 的第一条是 tool response，向前扩展直到包含其对应的 assistant tool_call
        ...
        # 清理 tail 中旧的图片数据，只保留最后一张
        last_image_idx = -1
        for i, msg in enumerate(tail):
            if msg.get("role") == "tool" and isinstance(msg.get("content"), list):
                last_image_idx = i
        for i, msg in enumerate(tail):
            if msg.get("role") == "tool" and isinstance(msg.get("content"), list) and i < last_image_idx:
                # 将图文混合内容替换为纯文本，移除图片
